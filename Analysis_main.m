...
switch choice
    case 'Mixed session'
        disp('Processing mixed-type data...');
        DLC_spatial(design_mat, data_path);
        disp('DLC analysis done!');
        PyControl_mixed(design_mat, data_path);
        disp('PyControl analysis done!');
        FormStandardData_mixed(design_mat, data_path);
    ...
    case 'Spatial learning'
        ...
        dFF_event_locked(design_mat, data_path, 'spatial_learning_58.mat', 1)

        %dFF_parameters(design_mat, data_path);
        %dFF(design_mat, data_path);
        %disp('dFF calculation done');
        ...
        %FPAnalysis_entry_aligned(design_mat, data_path);
end
